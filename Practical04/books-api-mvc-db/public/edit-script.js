...
editBookForm.addEventListener("submit", async (event) => {
  ...
  console.log("Edit form submitted for book ID:", bookIdInput.value); // Log the book ID being edited

  // Collect updated data from form fields (editTitleInput.value, editAuthorInput.value)
  ...
  try{
    // Make a PUT request to the API endpoint for updating the book
    ...
    if (response.status === 200) {
      ...
      console.log("Updated Book:", responseBody);
        ...
        // Handle the case where the book to update was not found
        messageDiv.textContent = `Error: Book not found. ID: ${bookIdInput.value}`;
        messageDiv.style.color = "red";
        console.error("Book Not Found:", responseBody);
    } else {
        ...
        messageDiv.textContent = "An unexpected error occurred while updating the book.";
        messageDiv.style.color = "red";
    }
});
